...
describe('decideNicehash - price tracking obeys NiceHash step/cooldown', () => {
  ...
  it('#B (v1.18.15): trims toward the fill line in capped 200 steps, even at full delivery', () => {
    // The fill line is trustworthy now that #E filters dust + the thin bottom
    // slice, so sitting ~900 above it is pure overpay. Descend one capped step.
    const actions = decideNicehash({
      ...BASE,
      providerActive: true,
  ...
  it('#B: trims when NOT delivering on target too (unchanged)', () => {
    const actions = decideNicehash({
      ...BASE,
      providerActive: true,
      desiredPriceSatPerPhDay: 47_259,
